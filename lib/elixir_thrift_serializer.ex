defmodule ElixirThriftSerializer do
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      defmodule ThriftSerializerStruct do
        ...
        def deserialize(record_binary, model) do
          ...
          try do
            with({:ok, memory_buffer_transport} <- :thrift_memory_buffer.new(record_binary),
                 ...
                 ThriftSerializerStruct.to_elixir(record, struct_definition)
            end
        ...
        def serialize(struct_to_serialize, model) do
            ...
            with({:ok, tf} <- :thrift_memory_buffer.new_transport_factory(),
                ...
                {:ok, binary_protocol} <- pf.()) do
                  ...
                  proto = ThriftSerializerStruct.to_erlang(struct_to_serialize, struct_definition)
